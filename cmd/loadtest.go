...
It is mandatory that the proxy APIs provided should forward the request to the targeted API asynchronously, with the ability to delay the request.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for p, ops := range proxiesToOperation {
				for _, o := range ops {
					...
					if !ok {
						return fmt.Errorf("error for --%s: %s is not separated by `:`", flagProxiesToOperation, ops)
					}
					var err error
	...
	cmd.Flags().VarP(anyflag.NewValue(
		...
		&proxiesToOperation,
		nil),
		flagProxiesToOperation, "p", "Map a proxy API to one or multiple operations. E.g. localhost:3000=POST:pet")
	...
	//loadTestCmd.Flags().StringP(flagMethod, "o", "POST", "The method to use on the target endpoint, e.g. POST, PUT, GET")
